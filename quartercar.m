...
function a = h(t, d)
%Genererar funktionen h(t) (backens form)
...
% så stoppas programmet istället efter n iterationer
    ...
    for i = 1:n
        F = transfer_functions(gissning(1), gissning(2), 0.71, 1);
...
% Uppgift 2
    ...
    [t_lista_2, eulerlista_2] = Euler(2000, 5*10^-4, [0, 0, 0, 0]', d);

    figure(f_3);
    plot(t_lista_1, eulerlista_1(2, :), "r-");
    hold on 
    plot(t_lista_2, eulerlista_2(2, :), "b-");
    plot(t, y(:,2), "g-")
...
function dummy = uppgift4(d)
    ...
    flera_numeriska_losningar(d, skalfaktorer1, "E", delta_tmax, 1);
